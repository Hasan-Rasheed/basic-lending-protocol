...
pub fn process_borrow(ctx: Context<Borrow>, amount:u64) -> Result<()>{
    ...
    match ctx.accounts.mint.to_account_info().key() {
        key if key == user.usdc_address => {
            ...
            let accrued_interest : u64 = calculate_accrued_interest(user.deposited_sol, bank.interest_rate, user.last_updated)?;
            total_collateral = (sol_price.price as u128 * (accrued_interest + user.deposited_sol) as u128) as u64;
        }
        _ => {
            let usdc_feed_id = get_feed_id_from_hex(USDC_USD_FEED_ID)?;
            let usdc_price = price_update.get_price_no_older_than(&Clock::get()?, MAX_AGE, &usdc_feed_id)?;
            let price: u128 = usdc_price.price as u128;
            let deposited: u128 = user.deposited_usdc as u128;
            total_collateral = (price *  deposited) as u64;
                }
    }
    let borrowable_amount = total_collateral.checked_mul(bank.liquidation_threshold).unwrap();
    if borrowable_amount < amount {
        return Err(ErrorCode::BorrowAmountExceedsLimit.into());
    }
