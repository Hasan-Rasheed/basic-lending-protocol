...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()>{
    ...
    match ctx.accounts.collateral_mint.to_account_info().key() {
        key if key == user.usdc_address => {
            let new_usdc_value : u64 = calculate_accrued_interest(user.deposited_usdc, collateral_bank.interest_rate, user.last_updated)?;   
            total_collateral = usdc_price.price as u64 * new_usdc_value;        
            let new_sol_value : u64 = calculate_accrued_interest(user.borrowed_sol, borrowed_bank.interest_rate, user.last_updated_borrowed)?;
            total_borrowed = sol_price.price as u64 * new_sol_value;
        },    
    _ => {
        let new_sol_value : u64 = calculate_accrued_interest(user.deposited_sol, collateral_bank.interest_rate, user.last_updated)?;
        total_collateral = sol_price.price as u64 * new_sol_value;
        let new_usdc_value : u64 = calculate_accrued_interest(user.borrowed_usdc, borrowed_bank.interest_rate, user.last_updated_borrowed)?;
    ...
    let liquidator_amount = ((liquidation_amount as f64 * collateral_bank.liquidation_bonus as f64) + liquidation_amount as f64) as u64;
